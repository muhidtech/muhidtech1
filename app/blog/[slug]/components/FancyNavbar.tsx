...
export default function FancyNavbar({
  ...
  const handleCategoryClick = (category: string) => {
    ...
    // Navigate to /blog with category query param
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 bg-black/20 backdrop-blur-2xl shadow-md z-50">
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
        {/* Logo / Brand */}
        <Link href="/" className="text-cyan-400 font-extrabold text-2xl tracking-wide hover:text-cyan-500 transition">
          MuhidTech
        </Link>

        {/* Search - show on md and above */}
        ...
        <div className="relative text-white">
          ...
          {/* Dropdown Menu */}
          {dropdownOpen && (
            <ul className="absolute right-0 mt-2 w-48 bg-gray-900 border border-gray-700 rounded-md shadow-lg overflow-hidden z-50">
        ...
        <div className="md:hidden ml-4 relative">
          ...
          {/* Mobile Dropdown */}
          {dropdownOpen && (
            <ul className="absolute right-0 top-10 w-48 bg-gray-900 border border-gray-700 rounded-md shadow-lg overflow-hidden z-50">
              {["All", ...categories].map((cat) => (
                <li key={cat}>
                  <button
                    onClick={() => handleCategoryClick(cat)}

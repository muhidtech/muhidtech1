    ...
    <>
      <nav className="fixed top-0 left-0 right-0 bg-black/20 backdrop-blur-2xl shadow-md z-50">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <Link
            href="/"
            className="bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 bg-clip-text text-transparent text-2xl font-bold"
          >
            MuhidTech
          </Link>
          ...
          <div className="hidden md:block flex-1 mx-8">
           ...
           {/* Mobile search icon only (hidden on md+) */}
          <button
            onClick={() => setShowMobileSearch((prev) => !prev)}
            className="block md:hidden ml-4 p-2 rounded-md hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-cyan-500"
          ...
          {/* Category Dropdown - always visible (desktop + mobile) */}
      ...
      {/* Mobile Search Input */}
      <div
        className={`md:hidden transition-all duration-300 ease-in-out bg-black/30 backdrop-blur-md px-6 ${
          showMobileSearch ? "max-h-32 py-4 mt-15" : "max-h-0 overflow-hidden py-0"
        }`}
      >
        <input
          type="text"
          ...
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Search blog posts..."
          className="w-full rounded-md border border-gray-700 bg-black/40 px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition"
        />
      </div>
    </>
  );
